 ...
 */
class RemeshSocialMenuDirective extends ViewDirective  {
	public static function renderMenuItem($menuItem) {
		...
		$output = "\t\t\t\t<a style='background-image:url({$imageURL})' href='{$menuItem['url']}' $target $attrs $submenu rel='{$menuItem['rel']}' class='$classes'>{$menuItem['title']}</a>\n";

		return $output;
	}

	public static function OutputMobileMenu($slug) {
		global $wp;
	...
	public function execute($args) {
		...
		return "<?php echo Remesh\\Directives\\RemeshSocialMenuDirective::OutputMobileMenu('{$args[0]}'); ?>";
	}
}